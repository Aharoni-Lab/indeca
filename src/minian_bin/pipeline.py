...
def pipeline_bin(
    ...
    if tau_init is not None:
        ...
        logger.info("Estimating initial tau values")
    ...
    for i_iter in trange(max_iters, desc="iteration"):
        # 2.1 deconvolution
        logger.info(f"Starting iteration {i_iter+1}/{max_iters}")
        res = []
        for icell, y in tqdm(
        ...
        if len(metric_prev) > 0:
            ...
            if np.abs(S - S_best).sum() < 1:
                logger.info(f"Converged at iteration {i_iter+1} by S similarity")
                break
            # trapped
