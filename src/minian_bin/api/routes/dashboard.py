...
try:
    from minian_bin.api.dashboard_adapter_fixed import (
        get_session_data,
        get_all_sessions,
    )
...
async def get_sessions():
    ...
    for session_id, clients in connection_manager.session_connections.items():
        sessions.append(SessionInfo(session_id=session_id, client_count=len(clients)))
...
async def get_session(session_id: str = Path(...)):
    ...
    return SessionInfo(session_id=session_id, client_count=client_count)
...
async def create_session():
    ...
    return SessionInfo(session_id=session_id, client_count=0)
...
async def get_dashboard_data(session_id: str = Path(...)):
    ...
    if not HAS_FIXED_ADAPTER:
        raise HTTPException(
            status_code=501, detail="Fixed dashboard adapter not available"
        )
...
async def websocket_endpoint(
    websocket: WebSocket, client_id: str = Query(None), session_id: str = Query(None)
):
    """WebSocket endpoint for dashboard connections."""
    try:
        client_id = await connection_manager.connect(
            websocket=websocket, client_id=client_id, session_id=session_id
        )
        ...
        while True:
            try:
                ...
                await connection_manager.send_personal_message(
                    {"event": "echo", "data": data}, client_id
                )
